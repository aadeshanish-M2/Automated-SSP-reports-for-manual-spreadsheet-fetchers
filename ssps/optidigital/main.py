#!/usr/bin/env python3
"""
Opti Digital (pbs.optidigital.com) → Google Sheets daily sync.

Pure API integration: OAuth2 password-flow token, then GET
/api/v1/reports/site/csv for MTD (date, site, impressions, spend per day
per site). Syncs to the destination sheet in the uniform format
(Domain | Date | Revenue | Impression | CPM). Previous-month history
preserved. No browser needed.
"""
...
SPREADSHEET_ID = os.environ.get("SHEETS_SPREADSHEET_ID") or "1qmFUDAMj2pPdjI914BuKqTenQ4bkZmnItfXSI29X1vc"
SHEET_NAME     = "Sheet1"
SCOPES         = ["https://www.googleapis.com/auth/spreadsheets"]

API_BASE = "https://pbs.optidigital.com/api"

# CSV headers (confirmed): "date","site","impressions","spend"
DATE_COL_CANDIDATES = ["date", "Date"]
SITE_COL_CANDIDATES = ["site", "Site"]
IMPR_COL_CANDIDATES = ["impressions", "Impressions"]
REV_COL_CANDIDATES  = ["spend", "revenue", "Revenue"]
ECPM_COL_CANDIDATES = ["ecpm", "eCPM"]   # not provided; CPM derived from totals
...
_REQUIRED_KEYS = ("optidigital_username", "optidigital_password")
...
def download_optidigital_csv(username: str, password: str) -> Path:
    ...
    if r.status_code != 200:
        sys.exit(f"ERROR: Opti Digital auth failed ({r.status_code}): {r.text[:300]}")
    ...
    tmp.write_text(rr.text)
    log(f"CSV saved → {tmp} ({len(rr.text.splitlines()) - 1} data rows)")
    return tmp
...
def process_csv(csv_path: Path) -> pd.DataFrame:
    ...
    out = pd.DataFrame({
        ...
        "CPM":         (pd.to_numeric(df[ecpm_col], errors="coerce").fillna(0)
                        if ecpm_col is not None else 0.0),
    })
...
def main():
    log("=== Opti Digital → Google Sheets daily sync ===")
    cfg   = load_config()
    creds = load_service_account()

    csv_path = download_optidigital_csv(cfg["optidigital_username"], cfg["optidigital_password"])
