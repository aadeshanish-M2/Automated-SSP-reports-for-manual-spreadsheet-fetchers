#!/usr/bin/env python3
"""
NoBid (insights.nobid.io) → Google Sheets daily sync.

Logs in via Playwright, opens the saved "Daily Report - ALL" earnings report,
sets the date range to MTD, runs the report, downloads the CSV via the
cloud-download icon, and syncs to the destination sheet in the uniform
...
SPREADSHEET_ID = os.environ.get("SHEETS_SPREADSHEET_ID") or "1ovACOOFQMZRyYlfZnH2az7sXkS3Pb8sCDSLGG-vnHNo"
SHEET_NAME     = "Sheet1"
SCOPES         = ["https://www.googleapis.com/auth/spreadsheets"]

LOGIN_URL   = "https://insights.nobid.io/#/login"
REPORTS_URL = "https://insights.nobid.io/#/pubreports"
REPORT_NAME = "Daily Report - ALL"

# CSV headers (confirmed): date,site_id,site_name,tldomain,impressions,net_revenue,avg_cpm
DATE_COL_CANDIDATES = ["date", "Date", "Day"]
SITE_COL_CANDIDATES = ["site_name", "Site", "site", "tldomain"]
IMPR_COL_CANDIDATES = ["impressions", "Impressions", "#Impressions"]
REV_COL_CANDIDATES  = ["net_revenue", "Revenue", "$Revenue", "revenue"]
ECPM_COL_CANDIDATES = ["avg_cpm", "CPM", "$Avg CPM", "eCPM"]
...
_REQUIRED_KEYS = ("nobid_username", "nobid_password")
...
def download_nobid_csv(username: str, password: str) -> Path:
    ...
    with sync_playwright() as p:
        ...
        log(f"Logged in. Opening Earning Reports → '{REPORT_NAME}'…")
        try:
            ...
            browser.close()
            sys.exit(f"ERROR: Could not open the saved report.\nDetail: {e}")

        # Set a rolling ~30-day range via the native date inputs (spans the month
        # boundary so the previous month's final day is refreshed once it's no
        # longer "today").
        today = date.today()
        first = today - timedelta(days=30)
        log(f"Setting date range {first.isoformat()} → {today.isoformat()}…")
        try:
            page.fill('#start_date', first.isoformat())
            page.fill('#end_date', today.isoformat())
            page.wait_for_timeout(500)
        except PlaywrightTimeoutError as e:
            browser.close()
            sys.exit(f"ERROR: Could not set date range.\nDetail: {e}")

        log("Clicking Run Report…")
        try:
            page.locator('button[aria-label="Run Report"]').click(timeout=15_000)
        except PlaywrightTimeoutError as e:
            browser.close()
...
def main():
    log("=== NoBid → Google Sheets daily sync ===")
    cfg   = load_config()
    creds = load_service_account()

    csv_path = download_nobid_csv(cfg["nobid_username"], cfg["nobid_password"])
